...
const CreateIssue = () => {
  ...
  const [formData, setFormData] = useState({
    title: '',
    description: '',
    category: 'Pothole', // Default category selection
    location: ''
  });
  ...
  // Handle standard text input modifications
  ...
  // Submit form data to Spring Boot
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    setMessage({ type: '', text: '' });

    // 🟢 Critical element: Using FormData to carry both text and binary image files
    ...
    try {
      // POST endpoint for tracking issues (assumes authentication is bypassed or handled)
      const response = await axios.post('http://localhost:8080/issues', data, {
        headers: {
      ...
      // Redirect back to home layout after a brief success notice
      setTimeout(() => {
        navigate('/');
      }, 2000);
      ...
      setMessage({ 
        type: 'error', 
        text: error.response?.data?.message || 'Failed to submit issue. Please check your backend connection.' 
      });
    } finally {
      setLoading(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="e.g., 4th Block, Sector 2, near City Park"
            className="w-full bg-slate-900 border border-slate-800 rounded px-4 py-2 text-white focus:outline-none focus:border-emerald-500 transition-colors"
          />
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 text-white font-bold py-2.5 rounded transition duration-200 mt-2 shadow-lg shadow-emerald-950/20"
        >
